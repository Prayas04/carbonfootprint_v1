...
async def get_dashboard_data(db: AsyncSession, user: User) -> DashboardResponse:
    ...
    # --- Global Impact & Daily Insight ---
    # Consolidate 3 queries into 1 using conditional aggregation
    result = await db.execute(
        select(
            func.sum(TransitEvent.impact_kg_co2e),
            func.sum(case((TransitEvent.impact_kg_co2e < 0, 1), else_=0)),
            func.sum(
                case(
                    (
                        TransitEvent.impact_kg_co2e < 0,
                        func.abs(TransitEvent.impact_kg_co2e),
                    ),
                    else_=0,
                )
            ),
    ...
    row = result.one()
    total_co2e = row[0] or 0.0
    green_count = row[1] or 0
    total_savings = row[2] or 0.0
    ...
    message = (
        ...
        if total_savings > 0
        else "Log your first green activity (bike, walk, or bus) to start getting personalized insights! 🌍"
    )
    ...
    daily_insight = DailyInsight(
        ...
        co2_saved_today=round(total_savings * 0.1, 1)
        if total_savings > 0
        else 0,  # Simulated daily portion
    )
    ...
    return DashboardResponse(
        global_impact=GlobalImpact(
            total_co2e_kg=round(total_co2e, 1), trend_data=trend_data
        ),
